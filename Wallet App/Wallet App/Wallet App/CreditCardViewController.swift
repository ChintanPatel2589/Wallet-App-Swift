   ...
   // var app_obj = UIApplication.sharedApplication().delegate as AppDelegate
    ...
    func done() //Save Data Insert New Data
    {
        ...
        if isCreditCard
        {
            ...
            var result=api_Database.genericQueryforDatabase(DATABASENAME, query:NSString(format:"insert into creditCard(UID,BankName,Name,Category,Type,CardNumber,ExpMonth,ExpYear,CCVCode,Pin,iBankingLogin,loginPassword,Phone,Email,Notes) values('%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@')",NSUUID.UUID().UUIDString,api_Database.encryptedString(txtBankName.text, KEYEncryption),api_Database.encryptedString(txtOwnerName.text, KEYEncryption),"Credit Card",api_Database.encryptedString(txtType.text, KEYEncryption),api_Database.encryptedString(txtNumber.text, KEYEncryption),api_Database.encryptedString(txtExpirationMonth.text, KEYEncryption),api_Database.encryptedString(txtExpirationYear.text, KEYEncryption),api_Database.encryptedString(txtCCV.text, KEYEncryption),api_Database.encryptedString(txtPin.text, KEYEncryption),api_Database.encryptedString(txtiBanking.text, KEYEncryption),api_Database.encryptedString(txtPass.text, KEYEncryption),api_Database.encryptedString(txtSupportPhone.text, KEYEncryption),api_Database.encryptedString(txtContactEmail.text, KEYEncryption),api_Database.encryptedString(txtViewNote.text, KEYEncryption)) as String)
